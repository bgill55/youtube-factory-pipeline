...
class VisualAssetAgent:
    ...
    def _log(self, msg):
        ...
        try:
            log.info(f"[Visual Agent] {safe_msg}")
        except Exception:
            pass
        if self._log_file:
            try:
    ...
    def _load_recently_used_clips(self):
        """Load the list of recently used B-roll clips from disk."""
        lib_path = self._get_broll_library_path()
        hist_path = os.path.join(lib_path, "recently_used_clips.json")
    ...
    def _save_recently_used_clip(self, clip_path):
        """Append a clip path to the recently used list and cap the size at 20."""
        lib_path = self._get_broll_library_path()
        hist_path = os.path.join(lib_path, "recently_used_clips.json")
        history = self._load_recently_used_clips()
        
        # Remove if already exists so it moves to the end (most recently used)
        if clip_path in history:
            history.remove(clip_path)
            
        history.append(clip_path)
        # Keep only the last 25 clips in history to cycle properly
        if len(history) > 25:
            history = history[-25:]
    ...
    def _search_local_broll(self, keywords):
        """Search local B-roll library for a matching clip by keyword overlap, avoiding duplicates."""
        index = self._load_broll_index()
        lib_path = self._get_broll_library_path()
        if not index or not os.path.exists(lib_path):
            return None
            
        # Ensure self.used_clips exists
        if not hasattr(self, 'used_clips'):
            self.used_clips = set()
        ...
        # Load history of recently used clips
        recently_used = self._load_recently_used_clips()
        ...
        # Keep candidates that have the best score or are very close (within 10% of max score)
        best_candidates = [c[1] for c in candidates if c[0] >= max_score * 0.9]
        
        # Filter 1: Exclude clips already used in the current run (strict filter)
        unused_current = [c for c in best_candidates if c not in self.used_clips]
        
        # If all candidates have been used in this run, fall back to all best candidates
        target_pool = unused_current if unused_current else best_candidates
        
        # Filter 2: Penalize/deprioritize clips that were recently used in past runs
        not_recent = [c for c in target_pool if c not in recently_used]
        
        import random
        # Choose from not_recent if available, otherwise choose from recently_used sorted by least recent
        if not_recent:
            chosen = random.choice(not_recent)
        else:
            # Sort recently_used by their index in the recently_used list (least recent is first)
            recent_sorted = sorted(target_pool, key=lambda x: recently_used.index(x) if x in recently_used else -1)
            # Pick from the oldest half of recently used clips to maximize variety
            pool_size = max(1, len(recent_sorted) // 2)
            chosen = random.choice(recent_sorted[:pool_size])
            
        # Mark as used in current run and update history
        self.used_clips.add(chosen)
        self._save_recently_used_clip(chosen)
    ...
    def query_llm_for_pexels_query(self, visual_description):
        ...
        try:
            ...
            log.info(f"[Visual Agent] LLM keyword extraction failed: {e}")
    ...
    def download_pexels_video(self, keywords, output_path, aspect_ratio="16:9", target_duration=None):
        ...
        log.info(f"[Visual Agent] Searching Pexels stock video for '{keywords}'...")
        ...
        log.info(f"[Visual Agent] Downloading Pexels video from {best_link} to {output_path}...")
        self.download_file(best_link, output_path)
        log.info(f"[Visual Agent] Downloaded Pexels video saved successfully to {output_path}")
    ...
    def generate_image_fal_video(self, prompt, output_path, aspect_ratio="16:9"):
        ...
        log.info(f"[Visual Agent] Submitting to Fal.ai queue ({model_name}) with prompt: '{prompt[:100]}...'")
        ...
        log.info(f"[Visual Agent] Polling Fal.ai task {request_id}...")
            ...
            if status_resp.status_code == 200:
                ...
                if status == "COMPLETED":
                    ...
                    if res_resp.status_code == 200:
                        ...
                        video_url = res_data.get("video", {}).get("url")
                        if video_url:
                            log.info(f"[Visual Agent] Downloading video from Fal.ai: {video_url}...")
                            self.download_file(video_url, output_path)
                            return
                    ...
                    log.info(f"[Visual Agent] Fal.ai status: {status} (Queue position: {queue_position}). Retrying in {poll_interval}s...")
            else:
                log.info(f"[Visual Agent] Warning: Status poll returned {status_resp.status_code}. Retrying...")
    ...
    def query_llm_for_prompt(self, visual_description):
        ...
        try:
            ...
            log.info(f"[Visual Agent] query_llm_for_prompt failed: {e}")
        ...
        # ==================== EXPLICIT ASSET TAG PARSING ====================

    ASSET_TAG_PATTERN = re.compile(
        r'\[Visual:\s*asset:(\w+):\s*([^\]]+)\]',
        re.IGNORECASE
    )

    def _parse_explicit_asset(self, visual_description):
        """
        Parse explicit asset tags from visual description.

        Supported formats:
        - [Visual: asset:screenshot:https://example.com/page]  -> web screenshot
        - [Visual: asset:video:https://example.com/demo.mp4]   -> remote video
        - [Visual: asset:video:local_demo.mp4]                 -> local video file
        - [Visual: asset:image:https://example.com/ui.png]     -> remote image
        - [Visual: asset:image:local_screenshot.png]           -> local image file

        Returns dict: {'type': 'screenshot|video|image', 'source': 'url|path', 'original': '...'} or None
        """
        match = self.ASSET_TAG_PATTERN.search(visual_description)
        if not match:
            return None

        asset_type = match.group(1).lower()
        asset_source = match.group(2).strip()

        # Validate type
        valid_types = {'screenshot', 'video', 'image'}
        if asset_type not in valid_types:
            self._log(f"Unknown asset type '{asset_type}' in tag: {match.group(0)}")
            return None
    ...
    def _fetch_explicit_asset(self, asset_info, output_path, scene_duration, aspect_ratio="16:9", run_dir=None, start_time=0.0):
        """
        Fetch/process an explicit asset and save to output_path.
        Returns (success: bool, final_path: str, method: str)
        """
        ...
        try:
            if asset_type == 'screenshot':
                if source.startswith(('http://', 'https://')):
                    if self.generate_screenshot_asset(source, output_path):
                        self.apply_browser_mockup(output_path, source)
                        return True, output_path, 'web_screenshot'
                return False, None, None

            elif asset_type == 'video':
                if source.startswith(('http://', 'https://')):
                    self._log(f"Downloading explicit video asset from {source}...")
                    self.download_file(source, output_path)
                    return self._transcode_asset(output_path, scene_duration, aspect_ratio, 'explicit_video_download', start_time=start_time)
                else:
                    local_path = self._resolve_local_asset_path(source, run_dir)
                    if local_path and os.path.exists(local_path):
                        self._log(f"Copying explicit local video asset from {local_path}...")
                        shutil.copy2(local_path, output_path)
                        return self._transcode_asset(output_path, scene_duration, aspect_ratio, 'explicit_video_local', start_time=start_time)
                    else:
                        self._log(f"Local video asset not found: {source}")
                        return False, None, None

            elif asset_type == 'image':
                if source.startswith(('http://', 'https://')):
                    self._log(f"Downloading explicit image asset from {source}...")
                    self.download_file(source, output_path)
                    return self._process_image_asset(output_path, aspect_ratio, 'explicit_image_download')
                else:
                    local_path = self._resolve_local_asset_path(source, run_dir)
                    if local_path and os.path.exists(local_path):
                        self._log(f"Copying explicit local image asset from {local_path}...")
                        shutil.copy2(local_path, output_path)
                        return self._process_image_asset(output_path, aspect_ratio, 'explicit_image_local')
                    else:
                        self._log(f"Local image asset not found: {source}")
                        return False, None, None

        except Exception as e:
            self._log(f"Explicit asset fetch failed: {type(e).__name__}: {e}")
            import traceback
            self._log(traceback.format_exc())

        return False, None, None

    def _resolve_local_asset_path(self, source, run_dir=None):
        """Resolve a local asset path relative to run_dir or workspace."""
        # Try as absolute path first
        if os.path.isabs(source) and os.path.exists(source):
            return source

        # Try relative to run_dir/assets/ (for per-run uploaded assets)
        if run_dir:
            candidates = [
                os.path.join(run_dir, "assets", source),
                os.path.join(run_dir, source),
            ]
            for c in candidates:
                if os.path.exists(c):
                    return c

        # Try relative to workspace/assets/
        workspace = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        candidates = [
            os.path.join(workspace, "assets", source),
            os.path.join(workspace, "workspace", "assets", source),
    ...
    def _transcode_asset(self, video_path, target_duration, aspect_ratio, method_name, start_time=0.0):
        import subprocess
        try:
            probe_cmd = [
                "ffprobe", "-v", "error", "-select_streams", "v:0",
                "DUMMY_ARG_JUST_TO_AVOID_ACCIDENTAL_MATCH"
            ]
            # Wait, let's keep original probe_cmd and probe logic but modify the trim filter logic
            probe_cmd = [
                "ffprobe", "-v", "error", "-select_streams", "v:0",
            ...
            if result.returncode != 0:
                self._log(f"ffprobe failed for {video_path}: {result.stderr}")
                return False, None, None
            ...
            if '/' in fps_str:
                ...
                src_fps = num / den if den else 30
            else:
            ...
            if src_duration > (start_time + target_duration):
                duration_filter = f"trim=start={start_time}:duration={target_duration},setpts=PTS-STARTPTS"
            else:
                rem_duration = max(0.1, src_duration - start_time)
                if rem_duration >= target_duration:
                    duration_filter = f"trim=start={start_time}:duration={target_duration},setpts=PTS-STARTPTS"
                else:
                    loops = int(target_duration / rem_duration) + 1
                    duration_filter = f"trim=start={start_time}:end={src_duration},loop=loop={loops}:size={int(rem_duration*src_fps)}:start=0,trim=duration={target_duration},setpts=PTS-STARTPTS"

            filter_complex = f"{scale_filter},{fps_filter},{duration_filter}"

            output_path = video_path.replace('.mp4', '_transcoded.mp4')
            cmd = [
            ...
            self._log(f"Transcoding explicit asset: {video_path} -> {output_path} ({target_w}x{target_h}@{target_duration:.1f}s)")
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=120)

            if result.returncode == 0 and os.path.exists(output_path):
                os.replace(output_path, video_path)
                return True, video_path, method_name
            else:
                self._log(f"FFmpeg transcode failed: {result.stderr[:300]}")
                if os.path.exists(output_path):
                    os.remove(output_path)
                return False, None, None
        except Exception as e:
            self._log(f"Transcode failed: {type(e).__name__}: {e}")
            return False, None, None

    def _process_image_asset(self, image_path, aspect_ratio, method_name):
        try:
            if aspect_ratio == "9:16":
                target_w, target_h = 1080, 1920
            else:
            ...
            return True, image_path, method_name
        except Exception as e:
            self._log(f"Image process failed: {type(e).__name__}: {e}")
            return False, None, None

    # ==================== END EXPLICIT ASSET TAG PARSING ====================
    ...
    def check_if_scene_needs_screenshot(self, visual_description, spoken_text, featured_links):
        ...
        for link in featured_links:
            ...
            if url:
                ...
                if clean_url in clean_desc or clean_url in clean_spoken:
                    log.info(f"[Visual Agent] Direct URL match found for {url} in visual/spoken text. Bypassing LLM check.")
                    return url
            ...
            if name:
                ...
                if len(clean_name) > 2:
                    ...
                    if re.search(pattern, clean_desc) or re.search(pattern, clean_spoken):
                        log.info(f"[Visual Agent] Link name match found: '{clean_name}' for URL {url}. Bypassing LLM check.")
                        return url
                ...
                if is_generic:
                    ...
                    if path_parts:
                        ...
                        if len(target_word) > 2:
                            ...
                            if re.search(pattern, clean_desc) or re.search(pattern, clean_spoken):
                                log.info(f"[Visual Agent] Fuzzy generic domain match found: '{target_word}' for URL {url}. Bypassing LLM check.")
                                return url
                else:
                    ...
                    if len(domain_parts) >= 2:
                        ...
                        if domain_word and len(domain_word) > 2:
                            ...
                            if re.search(pattern, clean_desc) or re.search(pattern, clean_spoken):
                                log.info(f"[Visual Agent] Fuzzy domain match found: '{domain_word}' for URL {url}. Bypassing LLM check.")
                                return url
                            ...
                            for w in words_in_text:
                                if w in domain_word:
                                    log.info(f"[Visual Agent] Sub-word domain match found: '{w}' in '{domain_word}' for URL {url}. Bypassing LLM check.")
                                    return url
            except Exception as e:
                log.info(f"[Visual Agent] Fuzzy parsing error for {url}: {e}")
        ...
        try:
            ...
            log.info(f"[Visual Agent] check_if_scene_needs_screenshot failed: {e}")
    ...
    def generate_screenshot_asset(self, url, output_path):
        ...
        """
        log.info(f"[Visual Agent] Generating web screenshot for {url}...")
        try:
            ...
            log.info(f"[Visual Agent] WARNING: Screenshot generation failed for {url}: {e}")
            return False
    ...
    def apply_browser_mockup(self, screenshot_path, url):
        ...
        try:
            ...
            log.info(f"[Visual Agent] Failed to apply browser mockup: {e}")
    ...
    def query_llm_for_thumbnail_meta(self, topic, summary):
        ...
        try:
            ...
            log.info(f"[Visual Agent] LLM thumbnail meta generation failed: {e}. Using defaults.")
...
OUTPUT: Raw JSON only."""
        ...
        try:
            ...
            log.info(f"[Visual Agent] Variant generation failed: {e}. Using defaults.")
    ...
    def draw_clickbait_text_overlay(self, image_path, text):
        ...
        if not os.path.exists(image_path):
            log.info(f"[Visual Agent] Image path {image_path} does not exist. Skipping overlay.")
            return
        ...
        try:
            ...
            log.info(f"[Visual Agent] Failed to draw clickbait text overlay: {e}")
    ...
    def generate_thumbnail_background(self, prompt, output_path, aspect_ratio="16:9"):
        ...
        if static_bg_dir and os.path.isdir(static_bg_dir):
            ...
            if bg_files:
                ...
                self._log(f"Thumbnail background: selected {os.path.basename(chosen)} from {len(bg_files)} options")
                try:
        ...
        log.info(f"[Visual Agent] Generating thumbnail background using provider '{image_provider}'...")
        ...
        for provider in providers:
            try:
                ...
                log.info(f"[Visual Agent] Thumbnail provider '{provider}' failed: {type(e).__name__}: {e}")
                if provider != "fallback":
                    log.info(f"[Visual Agent] Trying next provider...")
                continue
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    log.info(f"[Visual Agent] Pollinations queue busy (402). Retrying in {retry_delay}s... (Attempt {attempt+1}/{max_retries})")
                ...
                log.info(f"[Visual Agent] Network/Timeout error querying Pollinations: {re_err}. Retrying in {retry_delay}s... (Attempt {attempt+1}/{max_retries})")
                time.sleep(retry_delay)
                retry_delay += 2.5
    ...
    def generate_image_local_sd(self, prompt, output_path, aspect_ratio="16:9"):
            ...
            try:
                ...
                if not os.path.exists(local_lora_path):
                    ...
                    if os.path.exists(desktop_lora_path):
                        log.info(f"[Visual Agent] LoRA file '{filename}' found in local cache: {desktop_lora_path}. Copying to Stable Diffusion...")
                        shutil.copy(desktop_lora_path, local_lora_path)
                        log.info(f"[Visual Agent] Copied LoRA successfully.")
                    else:
                        log.info(f"[Visual Agent] LoRA file '{filename}' not found locally or in cache. Downloading from {lora_url}...")
                            ...
                            for chunk in response.iter_content(chunk_size=1024*1024): # 1MB chunks
                                if chunk:
                                    ...
                                    if total_size > 0:
                                        ...
                                        log.info(f"[Visual Agent] LoRA Download: {percent:.1f}% ({downloaded / (1024*1024):.1f}MB / {total_size / (1024*1024):.1f}MB)")
                                    else:
                                        log.info(f"[Visual Agent] LoRA Download: {downloaded / (1024*1024):.1f}MB downloaded")
                        
                        log.info(f"[Visual Agent] LoRA download complete: {local_lora_path}")
                    
                    # 4. Refresh LoRAs in Stable Diffusion
                    try:
                        log.info("[Visual Agent] Refreshing Stable Diffusion WebUI LoRAs...")
                        refresh_url = f"{base_url}/sdapi/v1/refresh-loras"
                        requests.post(refresh_url, timeout=15)
                        log.info("[Visual Agent] Stable Diffusion WebUI LoRAs refreshed successfully.")
                    except Exception as re:
                        log.info(f"[Visual Agent] Warning: Failed to refresh SD WebUI LoRAs via API: {re}")
                else:
                    log.info(f"[Visual Agent] LoRA file '{filename}' already exists locally. Skipping download.")
                ...
                log.info(f"[Visual Agent] LoRA successfully injected. Final prompt: {final_prompt}")
                
            except Exception as le:
                log.info(f"[Visual Agent] Error handling LoRA integration: {le}. Proceeding with original prompt.")
        ...
        log.info(f"[Visual Agent] Querying local Stable Diffusion (A1111) at: {url}...")
        ...
        with open(output_path, "wb") as f:
            f.write(image_data)
        log.info(f"[Visual Agent] Successfully saved local Stable Diffusion image to {output_path}")
    ...
    def _run_inner(self, inputs):
        ...
        for scene in scenes:
            ...
            # Check for explicit asset tags in visual description FIRST
            explicit_asset = self._parse_explicit_asset(visual_desc)
            start_time = 0.0

            # If not found in description, check matching visual segment from script_output
            if not explicit_asset and isinstance(voice_output, dict):
                visual_segments = voice_output.get("visual_segments", [])
                for vs in visual_segments:
                    if vs.get("scene_index") == idx and vs.get("type") == "explicit_asset":
                        asset_tag = vs.get("asset_tag")
                        if asset_tag:
                            explicit_asset = self._parse_explicit_asset(asset_tag)
                            if explicit_asset:
                                asset_params = vs.get("asset_params") or {}
                                start_time = float(asset_params.get("start", 0.0))
                                self._log(f"Scene {idx}: Found explicit asset in visual_segments: start_time={start_time}")
                        break

            if explicit_asset:
                self._log(f"Scene {idx}: Found explicit asset tag: {explicit_asset['original_tag']}")
                # Determine output path based on asset type
                if explicit_asset['type'] in ('video',):
                    asset_output_path = output_video_path
                else:
                    asset_output_path = output_image_path
                scene_duration = scene.get("duration", 5.0)
                success, final_path, method = self._fetch_explicit_asset(explicit_asset, asset_output_path, scene_duration, aspect_ratio, run_dir, start_time=start_time)
                if success:
                    generated_assets.append({
                        "scene_index": idx,
                        "raw_description": visual_desc,
                        "detailed_prompt": f"Explicit {explicit_asset['type']}: {explicit_asset['source']}",
                    ...
                    self._log(f"Scene {idx}: Explicit asset fetch failed, falling back...")
